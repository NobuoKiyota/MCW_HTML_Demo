 ...
 * その武器か"None"かを選択する(最大6武器を同時装備する想定のテスト用ツール)。
 *
 * このコンポーネント自体はデータの保持と、選択内容→ShotPatternID配列への変換のみを行う
 * (PlayerController.tsを直接書き換えたりはしない)。実際の反映はBehaviorTestController.beginTest()が
 * playerControllerを解決した直後にresolveSelectedShotPatternIds()を呼び、
 * playerController.setExtraShotPatternIds()へ渡す形で行う
...
export class PlayerWeaponManager extends Component {
    ...
    // 選択中の武器ID一覧(Noneは除外)をWeapons.csv経由でShotPatternIDへ変換して返す。
    ...
    public resolveSelectedShotPatternIds(): string[] {
        const db = GameDatabase.instance;
        const selections: string[] = [
            GROUP1_WEAPON_IDS[this.group1Weapon - 1],
            GROUP2_WEAPON_IDS[this.group2Weapon - 1],
            GROUP3_WEAPON_IDS[this.group3Weapon - 1],
            GROUP4_WEAPON_IDS[this.group4Weapon - 1],
            GROUP5_WEAPON_IDS[this.group5Weapon - 1],
            GROUP6_WEAPON_IDS[this.group6Weapon - 1],
        ];
        ...
        for (const weaponId of selections) {
            ...
            if (weapon && weapon.shotPatternId) {
                result.push(weapon.shotPatternId);
            } else {
